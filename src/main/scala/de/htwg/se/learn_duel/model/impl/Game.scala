...
case class Game(
  ...
  override def addPlayer(player: PlayerTrait): Unit =
    players = players :+ player

  override def removePlayer(player: PlayerTrait): Unit =
    players = players.filter(_ != player)

  override def playerCount(): Int = players.size

  override def addQuestion(question: QuestionTrait): Unit =
    questions = questions :+ question

  override def removeQuestion(question: QuestionTrait): Unit =
    questions = questions.filter(_ != question)
