...
class Controller @Inject()(gameState: Game) extends ControllerTrait {
  ...
  val db: MongoDatabase = mongoClient.getDatabase("learn-duel")
  val collection = db.getCollection("learn-duel-col")
  ...
  override def nextPlayerName: Option[String] = {
    gameState.playerCount() match {
      case c if c < maxPlayerCount =>
        Some(Player.baseName + (gameState.playerCount + 1).toString)
      case _ => None
    }
  }
  ...
  override def onHelp(): Unit = {
    notifyObserversAndSaveUpdate(
      new UpdateData(UpdateAction.SHOW_HELP, gameState))
  }
  ...
  override def onClose(): Unit = {
    notifyObserversAndSaveUpdate(
      new UpdateData(UpdateAction.CLOSE_APPLICATION, gameState))
  }
  ...
  override def onAnswerChosen(input: Int): Unit = {
    ...
    var (player: Option[Player], userInput: Int) = input match {
      ...
      case x
          if (6 until 10 contains x) && gameState.players.lengthCompare(1) > 0 =>
        (Some(gameState.players(1)), input - 5) // FIXME magic number -> local mp will be removed anyway
      case _ => (None, input)
    }

    if (player.isDefined && !playerAnsweredQuestion(player.get,
                                                    currentQuestion.id)) {
  ...
  override def onSaveResult(): Unit = {
    val result = Result.create(gameState.players)
    val gameStateDoc = Document.apply(Json.toJson(result).toString())
  ...
  override def onLoadResults(): Unit = {
    ...
    val list: List[Result] = res.map(r => {
      val jsonVal = Json.parse(r.toJson())
      Json.fromJson[Result](jsonVal).asOpt
    }).filter(r => r.isDefined).map(r => r.get).toList

    notifyObserversAndSaveUpdate(new UpdateData(UpdateAction.SHOW_PREVIOUS_RESULTS, gameState, Some(list)))
  }
  ...
  protected def addPlayer(name: Option[String]): String = {
    var playerName = name match {
      case Some(n) => n
      case None =>
        nextPlayerName.getOrElse("<unknown>") // will not be used if None
    }

    if (gameState.playerCount == Game.maxPlayerCount) {
      throw TooManyPlayersException(
        "There are too many players to add another one")
    ...
    try {
      ...
      case e: Throwable =>
        throw ControllerProcedureFailed("Adding player failed: " + e.getMessage)
    }

    notifyObserversAndSaveUpdate(
      new UpdateData(UpdateAction.PLAYER_UPDATE, gameState))
  ...
  protected def removePlayer(name: String): Unit = {
    if (gameState.playerCount == 1) {
      throw NotEnoughPlayersException(
        "There are not enough players to remove one")
    }
    ...
    gameState.players.find(p => p.name == name) match {
      case Some(p) =>
        ...
        notifyObserversAndSaveUpdate(
          new UpdateData(UpdateAction.PLAYER_UPDATE, gameState))
      case None =>
        throw PlayerNotExistingException(s"Player '$name' does not exist")
    }
  }
        ...
        override def run(): Unit = {
          ...
          newTime match {
            case Some(time) =>
              ...
              notifyObserversAndSaveUpdate(
                new UpdateData(UpdateAction.TIMER_UPDATE, gameState))
            case _ =>
          }
        }
      },
  ...
  protected def nextQuestion(): Unit = {
    ...
    if (gameState.currentQuestion.isDefined) {
      ...
      noAnswerPlayers.foreach(p =>
        p.wrongAnswers = p.wrongAnswers :+ currentQuestion)
    }
    ...
    if (questionIter.hasNext) {
      ...
      notifyObserversAndSaveUpdate(
        new UpdateData(UpdateAction.SHOW_RESULT, gameState))
    }
  }
  ...
  protected def showGame(): Unit = {
    ...
    notifyObserversAndSaveUpdate(
      new UpdateData(UpdateAction.SHOW_GAME, gameState))
  }
