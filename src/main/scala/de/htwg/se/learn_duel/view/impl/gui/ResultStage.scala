...
import de.htwg.se.learn_duel.model.{Player, Result}
...
class ResultStage(
  ...
  scene = new Scene {
    ...
    root = new VBox {
            ...
            val wrongContainer: VBox = new VBox {
              ...
              if (p.wrongAnswers.nonEmpty) {
                ...
                p.wrongAnswers.foreach(q => {
                  val correctAnswer =
                    q.answers.find(a => a.id == q.correctAnswer).get
      ...
      val player: Player = result.players.max[Player] {
